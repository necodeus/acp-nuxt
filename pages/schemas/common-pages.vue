...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/c_pages', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.pagesIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.pagesIndex.title"
            :description="pageTexts.pagesIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
