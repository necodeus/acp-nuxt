...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/s_discount_types', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.shopDiscountTypesIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.shopDiscountTypesIndex.title"
            :description="pageTexts.shopDiscountTypesIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
