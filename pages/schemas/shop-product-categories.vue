...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/s_product_categories', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.shopProductCategoriesIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.shopProductCategoriesIndex.title"
            :description="pageTexts.shopProductCategoriesIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
