...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/s_delivery_providers', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.shopDeliveryProvidersIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.shopDeliveryProvidersIndex.title"
            :description="pageTexts.shopDeliveryProvidersIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
