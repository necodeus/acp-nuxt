...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/s_orders', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.shopOrdersIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.shopOrdersIndex.title"
            :description="pageTexts.shopOrdersIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
