...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/f_categories', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.forumCategoriesIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.forumCategoriesIndex.title"
            :description="pageTexts.forumCategoriesIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
