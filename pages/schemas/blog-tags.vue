...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/b_tags', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.blogTagsIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.blogTagsIndex.title"
            :description="pageTexts.blogTagsIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
