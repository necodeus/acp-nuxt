...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/b_categories', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.blogCategoriesIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.blogCategoriesIndex.title"
            :description="pageTexts.blogCategoriesIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
