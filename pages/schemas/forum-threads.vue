...
const { data } = await useAsyncData(
  'posts',
  () => $fetch('http://admin-api.localhost/api/v1/schemas/f_threads', {
    params: {
      page: page.value
    }
  }), {
...
<template>
  <Head>
    <title>{{pageTexts.forumThreadsIndex.title}}</title>
  </Head>
      ...
      <div class="flex justify-between">
        <div class="w-[calc(50%-2.5px)]">
          <InformativeBox
            ...
            :title="pageTexts.forumThreadsIndex.title"
            :description="pageTexts.forumThreadsIndex.description"
          />
        </div>
        <div class="w-[calc(50%-2.5px)]">
